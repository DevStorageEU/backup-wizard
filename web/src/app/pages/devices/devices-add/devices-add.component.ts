...
export class DevicesAddComponent implements OnInit {
  ...
  validateIP(ipAddressInputField: HTMLInputElement) {
    ...
    console.log(ipAddressInputField.style.border)
    if (regex.test(ipAddressInputField.value)) {
      //valid
      ipAddressInputField.style.border = "";
    } else {
      //invalid
      ipAddressInputField.style.border = "red 1px solid"
    }

  }

  copySSHKeyCommandToClipboard() {
    navigator.clipboard.writeText("echo \""+this.sshPubKey+"\" >> /root/.ssh/authorized_keys && echo 'Backup SSH-Key added successfully'");
    this.toastr.info("Copied command to clipboard.")
  }
